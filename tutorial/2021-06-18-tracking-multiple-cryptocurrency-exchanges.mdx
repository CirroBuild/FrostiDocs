...
- schema-less/-agnostic ingestion (i.e. adding new fields will dynamically
  create new columns)
...
Run the image with InfluxDB port (`9009`) open:

```bash
docker run -p 9000:9000 -p 8812:8812 -p 9009:9009 questdb/questdb
```

Open up the Console UI on port `9000` and create a basic table:

```questdb-sql
CREATE TABLE crypto(currency STRING,
                    exchange STRING,
                    price DOUBLE,
                    timestamp TIMESTAMP)
